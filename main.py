...
def main():
    ...
    st.write("Increase your shopping experience. Just upload an image or video and we will do the rest.")
    st.write("Aimed to increase D2C conversion")

    # Inputs
    uploaded_file = st.file_uploader("Upload a video or image", type=["mp4", "avi", "mov", "mkv", "jpg", "png", "jpeg"])
    if uploaded_file:
        if uploaded_file.type.startswith("video"):
            ...
            # Process video
            frames = process_video(temp_path, model, frame_skip=30)
            ...
            with st.spinner("Detecting objects..."):
                detected_objects, _ = detect_objects_yolov5(temp_path, model)
            ...
            if detected_objects:
                ...
                for obj_index in selected_objects:
                    ...
                    cropped_path = crop_and_save_object(image, bbox, tempfile.gettempdir(), obj_index)
                    st.image(cropped_path, caption=f"Cropped Object {obj_index + 1}")
